...
**A negative result, from the same failure shape.** Our taxonomy cites the ICLR 2026 *LLMs Get Lost in Multi-Turn Conversation* result — an average **39% drop** — so we built a degradation curve to find the onset turn: the same question at 1, 3, 5 and 8 turns, facts always in the first message, question always last, grader and needle byte-identical inside a family, fillers carrying no numbers and no policy words. Only the distance changes. The raw curve read 100% / 100% / 100% / **67%**, onset at turn 8 — precisely what we expected to find. It was our needle again. All three stored responses at 8 turns rejected the return correctly; one of them wrote *"that window has **already** closed — a standard return would not be accepted"*, and our regex had no slot for the adverb and no alternative for that phrasing. Re-graded offline against the same stored responses: **11 of 12 correct**, and the one remaining miss is also ours — markdown emphasis landing inside the verdict, *"you are **not** within the ... window"*, breaks the match. **In the three families we measured, there is no degradation out to eight turns.** That does not refute the published 39%: they measured under-specified sharded prompts across 200,000+ conversations, our fillers are contentless, so we measured *distance* and they measured *specification*. One model, one platform, three families, eight turns — it did not reproduce in our conditions. We would rather print that than the finding we went looking for.
